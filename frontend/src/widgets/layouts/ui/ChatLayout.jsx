...
/**
 * ChatLayout — структурный каркас страницы чата: раскладывает готовые
 * виджеты по сетке (layout-wrapper) и рисует панель комнат
 * (user-profile-sidebar — класс из исходной вёрстки, содержимое —
 * список комнат). Открыт/закрыт-состояние панели живёт снаружи
 * (в pages/ChatPage.jsx), т.к. его нужно менять не только кнопкой
 ...
 *
 * Раньше здесь также рендерились side-nav (иконочная навигация слева)
 * и chat-sidebar (список users/private-chat) — оба были удалены как
 * неиспользуемые, поэтому chatWindow теперь занимает всю доступную
 * ширину (см. app/styles/user-chat.css — .user-chat больше не
 * ужимается фиксированными по ширине соседними колонками).
 *
