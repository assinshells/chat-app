import { create } from "zustand";
import { loginRequest } from "@features/auth/login/api/login.api.js";
import { AuthSession } from "@shared/lib/authSession.js";

export const useLoginStore = create((set) => ({
  loading: false,
  error: null,

  login: async ({ login, password }, onSuccess) => {
    set({ loading: true, error: null });
    try {
      const data = await loginRequest({ login, password });
      // refreshToken/csrfToken прийшли як cookie (див. axios.js/backend);
      // у тілі відповіді — лише accessToken, він живе в пам'яті вкладки.
      AuthSession.setAccessToken(data.accessToken);

      // Стать і колір тепер обираються один раз — на формі реєстрації
      // (див. RegisterForm.jsx) і йдуть напряму в POST /api/auth/register,
      // тому тут більше не потрібні окремі PATCH-запити після логіну.
      onSuccess();
    } catch (err) {
      set({ error: err.message });
    } finally {
      set({ loading: false });
    }
  },

  clearError: () => set({ error: null }),
}));
